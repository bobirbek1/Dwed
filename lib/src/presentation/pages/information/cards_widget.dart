...
class CardsWidgetState extends State<CardsWidget> {
      ...
      child: Column(
        children: [
          ...
          Expanded(child: ListView.builder(itemBuilder: (ctx, index) {
            return ListTile(leading: Image.asset(list[index].path),title :Text( list[index].text, style: const TextStyle(fontWeight: FontWeight.w400, fontSize: 16),));
          },
          itemCount: list.length,)),
          const Padding(
            padding: EdgeInsets.only(bottom: 8.0, top: 8),
            child: ListTile(leading: Icon(Icons.add_circle_outline),
              title: Text('Karta qo’shish', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w400),),),
          ),
        ],
      ),
    );
  ...
  List<CartItem> getList() {
    return [
      CartItem(path: 'assets/images/uzcart_logo.png', text: '8600 **** **** 1578 - Card Name'),
      CartItem(path: 'assets/images/uzcart_logo.png', text: '8600 **** **** 1578 - Card Name'),
      CartItem(path: 'assets/images/humo_logo.png', text: '8600 **** **** 1578 - Card Name'),
    ];
  }
